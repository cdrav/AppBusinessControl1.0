...
router.post('/', authenticateToken, async (req, res) => {
    ...
    const [r] = await db.query('INSERT INTO clients (tenant_id, name, email, phone, address) VALUES (?, ?, ?, ?, ?)', [req.user.tenant_id, name, email, phone, address]);
    res.status(201).json({ message: 'Cliente creado', clienteId: r.insertId });
});
router.put('/:id', authenticateToken, async (req, res) => {
    const { name, email, phone, address } = req.body;
    await db.query('UPDATE clients SET name=?, email=?, phone=?, address=? WHERE id=? AND tenant_id=?', [name, email, phone, address, req.params.id, req.user.tenant_id]);
